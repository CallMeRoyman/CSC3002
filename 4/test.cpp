...
bool findKnightsTour(Grid<int> & board, int row, int col, int seq) {
   ...
   for (int i = 0; i < 8; i++){
      //cout << i << ' ' << seq << endl;
      //tempRow = getRow(row, i);
      //tempCol = getCol(col, i);
      if (findKnightsTour(board, getRow(row, i), getCol(col, i), seq)) {
         return true;
      }
   }
   board[row][col] = 0;
...
/*
   if (!findKnightsTour(board, row+2, col+1, seq+1)){
      if (!findKnightsTour(board, row+1, col+2, seq+1)){
         if (!findKnightsTour(board, row-1, col+2, seq+1)){
            if (!findKnightsTour(board, row-2, col+1, seq+1)){
               if (!findKnightsTour(board, row-2, col-1
   case , seq+1)){
                  if (!findKnightsTour(board, row-1, col-2, seq+1)){
                     if (!findKnightsTour(board, row+1, col-2, seq+1)){
                        if (!findKnightsTour(board, row+2, col-1, seq+1)){
                           ...
                           findKnightsTour(board, row+2, col-1, seq+1);
                        }
                     }else{
                        findKnightsTour(board, row+1, col-2, seq+1);
                     }
                  }else{
                     findKnightsTour(board, row-1, col-2, seq+1);
                  }
